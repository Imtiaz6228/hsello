...
import { listSellerApplications, reviewSellerApplication } from "../services/seller.service.js";
...
adminRouter.patch("/seller-applications/:id", requireStaff, asyncHandler(async (req, res) => {
    ...
    const application = await reviewSellerApplication(applicationId, req.auth.id, input.status, input.adminNotes);
    res.json({
        message: "Seller application reviewed successfully.",
        application
    });
}));
adminRouter.get("/users", requireRole(Role.SUPER_ADMIN), asyncHandler(async (_req, res) => {
...
adminRouter.patch("/users/:id/role", requireRole(Role.SUPER_ADMIN), asyncHandler(async (req, res) => {
