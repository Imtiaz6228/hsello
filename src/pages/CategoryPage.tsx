...
export function CategoryPage() {
  ...
  const siblings = useMemo(
    () =>
      categories
        .filter((item) => !item.parentSlug && item.slug !== slug)
        .slice(0, 8),
    [categories, slug],
  );
  const filteredProducts = useMemo(() => {
    ...
    const matches = products.filter((product) => {
      ...
      const inSubFilter = categoryMatches(
  ...
  if (loading)
    return (
      <main className="commerce-page">
        <MarketHeader />
        <p className="empty-state">Loading category…</p>
      </main>
    );
    ...
    <main className="commerce-page market-browse-page">
        ...
        schema={[
          {
            ...
            name: category.name,
            description: category.description,
            url: `${window.location.origin}/categories/${slug}`,
          },
          {
            "@context": "https://schema.org",
            "@type": "BreadcrumbList",
            itemListElement: [
              ...
              },
              {
                ...
                position: 3,
                name: category.name,
                item: `${window.location.origin}/categories/${slug}`,
              },
            ],
          },
        ]}
      ...
      <section className="category-landing category-market-heading">
        <div className="category-heading-copy">
          ...
          <div className="category-trust-line">
            <ShieldLine />
            <span>
              Every listing shows its delivery method, seller support window,
              and usage terms before purchase.
            </span>
          </div>
        </div>
      ...
      {children.length ? (
        <section
          className="category-specialty-grid"
          aria-label={`${category.name} specialties`}
        >
          {children.map((child) => {
      ...
      <section className="category-product-shell">
        <div className="category-assurance-row">
          <span>
            <BadgeCheck /> Moderated listings
          </span>
          <span>
            <Clock3 /> Delivery shown upfront
          </span>
          <span>
          ...
          <div className="filter-controls">
            ...
            <label>
              <select
                ...
                value={kind}
                onChange={(event) =>
                  setKind(event.target.value as "all" | "DOWNLOAD" | "SERVICE")
                }
              >
        ...
        {!filteredProducts.length ? (
          <div className="no-results">
            ...
            <span>
              Try another specialty, remove a filter, or return to the complete
              catalog.
            </span>
            <button
              type="button"
      ...
      <section className="internal-links">
        ...
        {siblings.map((item) => (
          <Link to={`/categories/${item.slug}`} key={item.slug}>
            {item.name}
            <ArrowRight />
          </Link>
        ))}
