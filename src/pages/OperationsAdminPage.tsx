...
export function OperationsAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    const paths: Record<Tab, string> = {
      ...
      chats: "/api/assistant/admin/chats",
  ...
  async function sendAdminChat(event: FormEvent) {
    ...
    setChatReply("");
    await post(activeChatId, `/api/assistant/admin/chats/${activeChatId}/reply`, { body: text });
  }
  ...
  async function promptAndAct(id: string, path: string, body: Record<string, unknown>, field: string, title: string, label = "Admin note") {
    const note = await requestText({ title, label, description: "This note is required for the audit record.", confirmLabel: "Confirm action", minLength: 5, tone: "danger" });
    if (!note) return;
    await act(id, path, { ...body, [field]: note });
  }

  async function promptAndPost(id: string, path: string, body: Record<string, unknown>, field: string, title: string, label = "Message") {
    const note = await requestText({ title, label, confirmLabel: "Send", minLength: 1 });
    if (!note) return;
    await post(id, path, { ...body, [field]: note });
  }
  ...
  return (
    <main className={`ops-admin admin-enterprise ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${darkMode ? "admin-theme-dark" : ""}`}>
      ...
      <section className="ops-main">
          ...
          <section className="ops-grid">
            {applications.length ? applications.map((app) => (
              <article className="ops-card" key={app.id}>
                ...
                <footer><button className="approve" disabled={busy === app.id} onClick={() => void act(app.id, `/api/admin/seller-applications/${app.id}`, { status: "APPROVED" })}><BadgeCheck size={14} /> Approve</button><button className="danger" disabled={busy === app.id} onClick={() => void promptAndAct(app.id, `/api/admin/seller-applications/${app.id}`, { status: "REJECTED" }, "adminNotes", "Reject seller application", "Reason for rejection")}><AlertTriangle size={14} /> Reject</button></footer>
              </article>
        ...
        {tab === "reports" && <SimpleRows rows={reports.map((report) => ({ id: report.id, title: `${report.product.name} · ${report.reason}`, meta: `${report.reporter.email} · ${report.details ?? "No details"}`, status: report.status, actions: <><button disabled={busy === report.id} onClick={() => void promptAndAct(report.id, `/api/admin/reports/${report.id}`, { status: "ACTIONED", removeProduct: false }, "adminNotes", "Mark report actioned")}>Mark actioned</button><button className="danger" disabled={busy === report.id} onClick={() => void promptAndAct(report.id, `/api/admin/reports/${report.id}`, { status: "ACTIONED", removeProduct: true }, "adminNotes", "Remove reported product", "Removal note")}>Remove product</button></> }))} />}
