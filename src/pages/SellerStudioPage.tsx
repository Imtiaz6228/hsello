...
import { SellerProductEditor, type SellerEditableProduct } from "../components/SellerProductEditor";
...
export function SellerStudioPage() {
  ...
  return (
    <main className="seller-pro-dashboard">
      ...
      {editingProduct ? <SellerProductEditor key={editingProduct.id} product={editingProduct} categories={categories} onClose={() => setEditingProduct(null)} onSaved={async (text) => { showMessage(text, "success"); await load(); }} /> : null}
      ...
      {open ? <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Create product"><form className="seller-product-form seller-pro-product-form" onSubmit={create}><button type="button" className="modal-close" onClick={() => setOpen(false)} aria-label="Close">×</button><span className="section-index">NEW PRODUCT</span><h2>Create a smooth, buyer-friendly listing.</h2><p className="modal-helper">Follow the path from market category to platform and then listing type.</p>
        ...
        <section className="seller-language-panel"><header><span>中文</span><div><strong>Chinese product content</strong><small>Displayed automatically when the buyer selects Simplified Chinese.</small></div></header><label><span>产品标题</span><input value={form.chineseTitle} onChange={(event) => setForm({ ...form, chineseTitle: event.target.value })} placeholder="中文产品标题" /></label><label><span>简短描述</span><input maxLength={240} value={form.chineseShortDescription} onChange={(event) => setForm({ ...form, chineseShortDescription: event.target.value })} /></label><label><span>完整描述</span><textarea rows={5} value={form.chineseDescription} onChange={(event) => setForm({ ...form, chineseDescription: event.target.value })} /></label></section>
        <section className="seller-create-essentials"><header><Tag /><div><strong>Price and delivery</strong><small>Only the essentials are required. You can edit every other detail later.</small></div></header><div className="form-grid three"><label><span>Price (USD)</span><input required type="number" min="0.50" step="0.01" value={form.priceUsd} onChange={(event) => setForm({ ...form, priceUsd: event.target.value })} /></label><label><span>Stock quantity</span><input required type="number" min={0} value={form.stockQuantity} onChange={(event) => setForm({ ...form, stockQuantity: Number(event.target.value) })} /></label><label><span>Delivery method</span><select value={form.deliveryMethod} onChange={(event) => setForm({ ...form, deliveryMethod: event.target.value })}><option>Manual delivery</option><option>Instant delivery</option><option>Automatic delivery</option><option>Digital download</option><option>Seller service</option></select></label></div></section>
        <details className="seller-create-advanced"><summary><SlidersHorizontal /><span><strong>Advanced options</strong><small>Optional pricing, inventory, SEO, and searchable details</small></span><ChevronDown /></summary><div className="seller-create-advanced-body">
          <section><h3>Pricing and inventory</h3><div className="form-grid three"><label><span>Sale price</span><input type="number" min="0" step="0.01" value={form.salePrice} onChange={(event) => setForm({ ...form, salePrice: event.target.value })} /></label><label><span>Wholesale price</span><input type="number" min="0" step="0.01" value={form.wholesalePrice} onChange={(event) => setForm({ ...form, wholesalePrice: event.target.value })} /></label><label><span>Price (CNY)</span><input type="number" min="0" step="0.01" value={form.priceCny} onChange={(event) => setForm({ ...form, priceCny: event.target.value })} /></label><label><span>Price (RUB)</span><input type="number" min="0" step="0.01" value={form.priceRub} onChange={(event) => setForm({ ...form, priceRub: event.target.value })} /></label><label><span>Discount %</span><input type="number" min={0} max={100} value={form.discountPercent} onChange={(event) => setForm({ ...form, discountPercent: Number(event.target.value) })} /></label><label><span>SKU</span><input value={form.sku} onChange={(event) => setForm({ ...form, sku: event.target.value })} /></label><label><span>Min / max order</span><span className="seller-inline-inputs"><input type="number" min={1} value={form.minimumOrder} onChange={(event) => setForm({ ...form, minimumOrder: Number(event.target.value) })} /><input type="number" min={1} value={form.maximumOrder} onChange={(event) => setForm({ ...form, maximumOrder: Number(event.target.value) })} /></span></label><label><span>Tags</span><input value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} placeholder="popular, instant, premium" /></label><label><span>After-sales hours</span><input type="number" min={12} max={8760} value={form.afterSalesServiceHours} onChange={(event) => setForm({ ...form, afterSalesServiceHours: Number(event.target.value) })} /></label></div></section>
          <section><h3>Searchable specifications</h3><div className="form-grid three">{([['brand','Brand'],['platform','Platform'],['region','Region'],['country','Country'],['server','Server'],['language','Language'],['productKind','Product type'],['condition','Condition'],['stockType','Stock type'],['duration','Duration'],['warranty','Warranty']] as const).map(([key,label]) => <label key={key}><span>{label}</span><input value={form[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} /></label>)}</div></section>
          <section><h3>Chinese SEO</h3><div className="form-grid two"><label><span>SEO 标题</span><input maxLength={70} value={form.chineseSeoTitle} onChange={(event) => setForm({ ...form, chineseSeoTitle: event.target.value })} /></label><label><span>SEO 描述</span><textarea maxLength={170} rows={2} value={form.chineseSeoDescription} onChange={(event) => setForm({ ...form, chineseSeoDescription: event.target.value })} /></label></div></section>
          <section><h3>Fulfillment</h3><div className="seller-option-toggles"><label><input type="checkbox" checked={form.instantDelivery} onChange={(event) => setForm({ ...form, instantDelivery: event.target.checked })} /> Instant delivery</label><label><input type="checkbox" checked={form.manualDelivery} onChange={(event) => setForm({ ...form, manualDelivery: event.target.checked })} /> Manual delivery</label><label><input type="checkbox" checked={form.digitalDownload} onChange={(event) => setForm({ ...form, digitalDownload: event.target.checked })} /> Digital download</label><label><input type="checkbox" checked={form.couponSupport} onChange={(event) => setForm({ ...form, couponSupport: event.target.checked })} /> Coupons supported</label></div><label><span>Buyer delivery note</span><textarea rows={3} value={form.deliveryNote} onChange={(event) => setForm({ ...form, deliveryNote: event.target.value })} /></label><label><span>Refund policy</span><textarea rows={3} value={form.refundPolicy} onChange={(event) => setForm({ ...form, refundPolicy: event.target.value })} /></label><label><span>Digital inventory rows</span><textarea rows={5} placeholder="One license, code, or deliverable per line. You can also add these later." value={form.inventoryLines} onChange={(event) => setForm({ ...form, inventoryLines: event.target.value })} /></label></section>
        </div></details>
        <button className="primary-button seller-submit-product" disabled={busy}><PackagePlus /> {busy ? "Creating product…" : "Create product and send for review"}</button>
      </form></div> : null}
    </main>
  );
}
