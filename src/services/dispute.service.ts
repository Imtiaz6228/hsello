...
const ACTIVE = [DisputeStatus.OPEN, DisputeStatus.UNDER_REVIEW, DisputeStatus.AWAITING_BUYER, DisputeStatus.AWAITING_SELLER];
...
export async function autoResolveExpiredDisputes(where: Record<string, unknown> = {}) {
  const now = new Date();
  const disputes = await prisma.dispute.findMany({
    where: { ...where, status: { in: ACTIVE }, autoCloseAt: { lte: now }, awaitingParty: { in: ["BUYER", "SELLER"] } },
  ...
  await prisma.$transaction(async (tx) => {
    for (const dispute of disputes) {
      ...
      await tx.dispute.update({
        where: { id: dispute.id },
        data: {
          status: favor === "BUYER" ? DisputeStatus.RESOLVED_BUYER : DisputeStatus.RESOLVED_SELLER,
...
export async function markDisputeTurn(orderId: string, author: { id: string; role: Role }, orderBuyerId: string, orderSellerIds: string[]) {
  const now = new Date();
  ...
  const data = party === "BUYER"
    ...
    : party === "SELLER"
      ? { lastSellerMessageAt: now, awaitingParty: "BUYER", autoCloseAt: responseDeadline(now), status: DisputeStatus.AWAITING_BUYER }
      : { lastAdminMessageAt: now, awaitingParty: null, autoCloseAt: null, status: DisputeStatus.UNDER_REVIEW };

  await prisma.dispute.updateMany({ where: { orderId, status: { in: ACTIVE } }, data });
}
